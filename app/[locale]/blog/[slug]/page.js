...
export default async function BlogArticlePage({ params }) {
  ...
  const articleSchema = {
    ...
    "author": {
      "@type": "Person",
      "name": authorName,
      "jobTitle": "Consultant Tehnic Senior",
      "worksFor": {"@type":"Organization","name":"Neofort BIZ","url":BASE},
      "image": a.authorPhoto ? `${BASE}${a.authorPhoto}` : `${BASE}/team/mihai-danalache.avif`
    },
    "publisher": {"@type":"Organization","name":"Neofort BIZ","url":BASE,
      ...
      <div style={{position:'relative',background:a.imageBg||'#1a1a1a',overflow:'hidden',maxHeight:'40vh',minHeight:'220px',display:'flex',flexDirection:'column',justifyContent:'flex-end'}}>
        ...
        <div style={{position:'relative',zIndex:2,padding:'0 24px 28px',maxWidth:'1100px',margin:'0 auto',width:'100%',boxSizing:'border-box'}}>
          <div style={{display:'flex',gap:'14px',alignItems:'center',marginBottom:'8px',flexWrap:'wrap'}}>
            ...
            <span style={{color:'rgba(255,255,255,.2)'}}>·</span>
            <ViewCounter slug={a.slugs.ro} />
            <span style={{color:'rgba(255,255,255,.2)'}}>·</span>
            <span style={{display:'inline-flex',alignItems:'center',gap:'6px'}}>
              <img src={a.authorPhoto||'/team/mihai-danalache.avif'} alt={a.author||'Mihai Dănălache'} width={20} height={20} style={{width:'20px',height:'20px',borderRadius:'50%',objectFit:'cover',border:'1px solid rgba(255,255,255,.4)'}} loading="lazy"/>
              <span style={{fontFamily:'Barlow Condensed,sans-serif',fontSize:'.58rem',letterSpacing:'.08em',color:'rgba(255,255,255,.7)'}}>{a.author||'Mihai Dănălache'}</span>
            </span>
          </div>
      ...
      <div style={{background:'#fff',padding:'48px 0 72px'}}>
        <div className="article-grid">
          ...
          <main style={{minWidth:0}}>
            ...
            {related.length > 0 && (
              <div style={{marginTop:'64px',paddingTop:'36px',borderTop:'1px solid #efefed'}}>
                <span style={{fontFamily:'Barlow Condensed,sans-serif',fontSize:'.58rem',letterSpacing:'.2em',textTransform:'uppercase',color:'#595959',display:'block',marginBottom:'20px'}}>{ui.related}</span>
                  ...
                  {related.map(r => {
                    ...
                    return (
                      <Link key={r.slugs.ro} href={`/blog/${rSlug}`}
                        ...
                        <p style={{fontSize:'.74rem',color:'#6b6b6b',lineHeight:1.6,margin:'0 0 10px'}}>{rex.slice(0,90)}…</p>
          ...
          <aside className="article-sidebar">
            ...
            {/* 1.5 Card autor */}
            <div className="sidebar-card">
              <span className="sidebar-label" style={{color:'#595959'}}>{'ro'===locale?'Autor':'en'===locale?'Author':'de'===locale?'Autor':'fr'===locale?'Auteur':'es'===locale?'Autor':'Autore'}</span>
            ...
            <div className="sidebar-card" style={{background:'#f0faf3',borderTopColor:'#25D366'}}>
              ...
              <a href="https://wa.me/40752443435" target="_blank" rel="noopener noreferrer"
                style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',background:'#075E35',color:'#fff',textDecoration:'none',padding:'10px 0',fontFamily:'Barlow Condensed,sans-serif',fontWeight:600,fontSize:'.6rem',letterSpacing:'.18em',textTransform:'uppercase'}}>
