...
export default async function ContactPage({ params }) {
  ...
  return (
    <>
      ...
      {/* ── MOBIL: echipă grid 2 col (neschimbat) ── */}
      <section className="lg:hidden py-16 border-b border-border">
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-2 gap-4">
            {TEAM.map(m => (
              <div key={m.name} className="flex flex-col border border-border p-4">
                <div style={{display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px'}}>
                  <div style={{width:'52px',height:'52px',borderRadius:'50%',flexShrink:0,overflow:'hidden',background:m.color,border:'1px solid #e8e8e4'}}>
                    {m.photo ? (
                      <img src={m.photo} alt={m.name} width={52} height={52}
                        style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'center top',display:'block'}}/>
                    ) : (
                      <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center'}}>
                        <span style={{fontFamily:'Barlow Condensed, sans-serif',fontWeight:600,fontSize:'1rem',letterSpacing:'.06em',color:'#fff'}}>{m.initials}</span>
                      </div>
                    )}
                  </div>
                  <div style={{minWidth:0}}>
                    <h3 style={{fontFamily:'Barlow Condensed, sans-serif',fontWeight:600,fontSize:'.88rem',lineHeight:1.2,color:'#1a1a1a',marginBottom:'3px',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{m.name}</h3>
                    <p style={{fontFamily:'Barlow Condensed, sans-serif',fontSize:'.62rem',letterSpacing:'.06em',color:m.color,margin:0,textTransform:'uppercase'}}>{m.role[locale] || m.role.ro}</p>
                  </div>
                </div>
                <div className="flex flex-col flex-1 w-full">
                  <a href={m.href} className="w-full text-center font-condensed font-semibold tracking-[0.04em] uppercase text-primary border border-border px-1 py-2 mb-2 hover:border-primary hover:bg-primary hover:text-white transition-all duration-200 block"
                    style={{fontSize:'clamp(0.48rem, 1.6vw, 0.6rem)',lineHeight:1.3}}>{m.phone}</a>
                  <a href={`mailto:${m.email}`} className="w-full text-center font-condensed font-semibold text-[0.6rem] tracking-[0.12em] uppercase text-muted border border-border px-2 py-2 hover:border-primary hover:text-primary transition-all duration-200 block">E-mail</a>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── MOBIL: hartă + date contact (neschimbat) ── */}
      <section className="lg:hidden py-16 border-b border-border">
        <div className="container mx-auto px-6">
          <div className="flex flex-col gap-10">
            <div className="flex flex-col">
              ...
              <div className="border border-border overflow-hidden" style={{minHeight:'320px'}}>
                <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2848.3!2d26.0859381!3d44.4429398!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x40b1ff4770adb5b1%3A0xa9d64b2f9d9b0f2a!2sStrada%20Theodor%20Aman%2011%2C%20Bucure%C8%99ti!5e0!3m2!1sro!2sro!4v1700000000000!5m2!1sro!2sro"
              ...
              <a href="https://maps.app.goo.gl/YcaANuqcmnzy14i1A" target="_blank" rel="noopener noreferrer"
                className="inline-block mt-4 font-condensed text-[0.68rem] tracking-[0.15em] uppercase font-semibold text-muted hover:text-primary transition-colors duration-200">{ui.maps_btn}</a>
            </div>
            <div className="flex flex-col gap-6">
              ...
              <div>
                ...
                <div className="border border-border">
                  {[{label:ui.dept_office,phone:'+40 21 528 06 61',href:'tel:+40215280661'},{label:ui.dept_quotes,phone:'+40 752 443 435',href:'tel:+40752443435'},{label:ui.dept_export,phone:'+40 752 443 439',href:'tel:+40752443439'},{label:ui.dept_tech,phone:'+40 752 443 431',href:'tel:+40752443431'},{label:ui.dept_service,phone:'+40 752 443 432',href:'tel:+40752443432'},{label:ui.dept_schedule,phone:'+40 758 990 048',href:'tel:+40758990048'}].map((item,i,arr) => (
                    <a key={item.label} href={item.href} className={`flex justify-between items-center px-4 py-2.5 hover:bg-light transition-colors duration-150 ${i<arr.length-1?'border-b border-border':''}`}>
              ...
              <div>
                ...
                <div className="border border-border">
                  {[{href:'mailto:oferte@neofort-biz.ro',label:ui.dept_quotes},{href:'mailto:comenzi@neofort-biz.ro',label:locale==='ro'?'Comenzi':locale==='en'?'Orders':locale==='de'?'Bestellungen':locale==='fr'?'Commandes':locale==='es'?'Pedidos':'Ordini'},{href:'mailto:service@neofort-biz.ro',label:ui.dept_service}].map((e,i,arr) => (
                    <a key={e.label} href={e.href} className={`flex justify-between items-center px-4 py-2.5 hover:bg-light transition-colors duration-150 group ${i<arr.length-1?'border-b border-border':''}`}>
                      ...
                      <span className="font-condensed text-[0.62rem] tracking-[0.12em] uppercase text-muted group-hover:text-primary transition-colors">{ui.email_section} →</span>
                    </a>
                  ))}
                </div>
              </div>
      ...
      {/* ── DESKTOP: 3 coloane — echipă | hartă | date contact ── */}
      <section className="hidden lg:block py-16 border-b border-border">
        <div className="container mx-auto px-6">
          <div style={{display:'grid', gridTemplateColumns:'240px 1fr 340px', gap:'40px', alignItems:'start'}}>

            {/* COL 1 — Echipă stivuită */}
            <div style={{display:'flex', flexDirection:'column', gap:'1px', background:'#e8e8e4', border:'1px solid #e8e8e4'}}>
              <div style={{background:'#fafaf8', padding:'14px 16px 10px', borderBottom:'1px solid #e8e8e4'}}>
                <span style={{fontFamily:'Barlow Condensed,sans-serif', fontSize:'.56rem', letterSpacing:'.2em', textTransform:'uppercase', fontWeight:600, color:'#aaa'}}>{ui.team_label}</span>
              </div>
              {TEAM.map((m, i) => (
                <div key={m.name} style={{background:'#fff', padding:'14px 16px', display:'flex', flexDirection:'column', gap:'12px'}}>
                  {/* Foto + nume */}
                  <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
                    <div style={{width:'44px', height:'44px', borderRadius:'50%', flexShrink:0, overflow:'hidden', background:m.color, border:'1px solid #e8e8e4'}}>
                      {m.photo ? (
                        <img src={m.photo} alt={m.name} width={44} height={44}
                          style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'center top',display:'block'}}/>
                      ) : (
                        <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center'}}>
                          <span style={{fontFamily:'Barlow Condensed,sans-serif',fontWeight:600,fontSize:'.9rem',color:'#fff'}}>{m.initials}</span>
                        </div>
                      )}
                    </div>
                    <div style={{minWidth:0}}>
                      <div style={{fontFamily:'Barlow Condensed,sans-serif',fontWeight:600,fontSize:'.82rem',color:'#1a1a1a',lineHeight:1.2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{m.name}</div>
            ...
            <div style={{display:'flex', flexDirection:'column'}}>
              <span className="sec-label">{ui.map_label}</span>
              <h2 className="font-condensed text-2xl font-semibold text-primary mb-5">{ui.map_title}</h2>
              <div className="border border-border overflow-hidden" style={{flex:1, minHeight:'460px'}}>
                <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2848.3!2d26.0859381!3d44.4429398!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x40b1ff4770adb5b1%3A0xa9d64b2f9d9b0f2a!2sStrada%20Theodor%20Aman%2011%2C%20Bucure%C8%99ti!5e0!3m2!1sro!2sro!4v1700000000000!5m2!1sro!2sro"
                  width="100%" height="100%" style={{border:0,display:'block',minHeight:'460px'}}
                  allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade"
                  title="Neofort BIZ — Str. Theodor Aman Pictor 11, Sector 1, București"/>
              </div>
              <a href="https://maps.app.goo.gl/YcaANuqcmnzy14i1A" target="_blank" rel="noopener noreferrer"
            ...
            <div style={{display:'flex', flexDirection:'column', gap:'24px'}}>
              ...
              <div>
                <p className="font-condensed text-[0.62rem] tracking-[0.2em] uppercase font-semibold text-muted mb-2">{ui.whatsapp}</p>
                <a href="https://wa.me/40752443435" target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 border border-border px-4 py-2.5 hover:border-[#25D366] transition-all duration-200 group">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="#25D366" className="shrink-0"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                  <div>
                    <span className="font-condensed text-[0.82rem] font-medium text-primary block">+40 752 443 435</span>
                    <span className="font-condensed text-[0.6rem] tracking-[0.1em] uppercase text-muted group-hover:text-[#25D366] transition-colors">Chat →</span>
