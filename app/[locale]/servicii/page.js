...
export default async function ServiciiPage({ params }) {
  ...
  const faqSchema = {
    ...
    'mainEntity': [
      { '@type':'Question',
        'name': locale==='ro'?'Ce include serviciul de montaj tâmplărie?':locale==='en'?'What does the window installation service include?':locale==='de'?'Was ist im Fenstermontageservice enthalten?':locale==='fr'?`Que comprend le service de pose de menuiserie?`:locale==='es'?`¿Qué incluye el servicio de instalación de ventanas?`:`Cosa include il servizio di installazione infissi?`,
