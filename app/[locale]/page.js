...
import { formatRatingParts } from '../../lib/formatRating.js';
...
const CONTENT = {
  ro: {
    ...
    stats: [['21+','Ani activitate neîntreruptă'],['nZEB','Certificate conformitate'],['50%+','Clienți recurenți'],['5.0★','52 Recenzii Google']],
  ...
  en: {
    ...
    stats: [['21+','Years of activity'],['nZEB','Compliance certificates'],['50%+','Returning clients'],['5.0★','52 Google Reviews']],
  ...
  de: {
    ...
    stats: [['21+','Jahre Erfahrung'],['nZEB','Zertifizierte Konformität'],['50%+','Stammkunden'],['5.0★','52 Google-Bewertungen']],
  ...
  fr: {
    ...
    stats: [['21+','Ans d\'activité'],['nZEB','Certifiés conformes'],['50%+','Clients fidèles'],['5.0★','52 Avis Google']],
  ...
  es: {
    ...
    stats: [['21+','Años de actividad'],['nZEB','Certificados conformes'],['50%+','Clientes recurrentes'],['5.0★','52 Reseñas Google']],
  ...
  it: {
    ...
    stats: [['21+','Anni di attività'],['nZEB','Certificati conformi'],['50%+','Clienti fedeli'],['5.0★','52 Recensioni Google']],
...
export default async function HomePage({ params }) {
  const { locale } = await params;
  const base = CONTENT[locale] || CONTENT.ro;
  const googleRating = await getGoogleRating();
  const c = { ...base, stats: [...base.stats.slice(0, -1), formatRatingParts(googleRating.rating, googleRating.count, locale)] };

  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(buildSchema(locale, c, googleRating)) }}/>
            ...
            {c.stats.map(([n,l],i)=>(
              i === 3 ? (
                <a key={l} href="https://maps.app.goo.gl/dvbPRmMwAroCPfSC8" target="_blank" rel="noopener noreferrer"
                  ...
                  <div style={{fontFamily:'Barlow Condensed,sans-serif',fontWeight:500,fontSize:'1.6625rem',color:'#8a5e00',lineHeight:1}}>{n}</div>
          ...
          <div className="steps-grid-home">
            {c.steps.map((s,i)=>(
              <div key={s.n} className="step-item" style={{padding:'36px 32px',borderRight:i<2?'1px solid #e8e8e8':'none'}}>
                <div aria-hidden="true" style={{fontFamily:'Barlow Condensed,sans-serif',fontWeight:300,fontSize:'clamp(1.6625rem, 4vw, 2.4625rem)',color:'#767676',lineHeight:1,marginBottom:'20px'}}>{s.n}</div>
